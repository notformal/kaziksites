...
let token=sessionStorage.getItem('casino_session')||'';
...
export const api={
 register:async data=>{const r=await request('/auth/register',{method:'POST',body:JSON.stringify(data)});token=r.token;sessionStorage.setItem('casino_session',token);return r},
 login:async data=>{const r=await request('/auth/login',{method:'POST',body:JSON.stringify(data)});token=r.token;sessionStorage.setItem('casino_session',token);return r},
 logout:async()=>{try{await request('/auth/logout',{method:'POST'})}finally{token='';sessionStorage.removeItem('casino_session')}},
