...
param([switch]$Apply)
$ErrorActionPreference='Stop';$root=(Resolve-Path (Join-Path $PSScriptRoot '../..')).Path;$taskName='VirtualCasinoNativeRuntime'
...
if($PSCmdlet.ShouldProcess($taskName,'Register current-user startup task')){Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Settings $settings -Description 'Nova Casino native runtime' -Force|Out-Null;Write-Host "Installed scheduled task '$taskName'."}
